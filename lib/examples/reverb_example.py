...
def main(vs, args):
    ...
    with master,  wv,rev, comp:
        ...
        master.add(rev) 
        master.add(comp)
        comp.set_params(1.2, 1.0)
        frames = wv.load_wavetable(0, "/sd/lib/data/piano_a2.wav", stride=1, max_frames=64, frame_size = 256)
        ...
        # 3. Configure Reverb
        rev.set_params(room_size=0.40, brightness=0.6, predelay_ms=215.0, transition_ms=0, mix=0.4)
