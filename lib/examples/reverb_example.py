...
def main(vs, args):
    ...
    with master,  wv, rev, comp:
        ...
        frames = wv.load_wavetable(0, "/sd/data/wavetable/Instruments/ESW Real - World Japanese Guitar.wav", stride=4, max_frames=64)
        ...
        for i in range(4):
          ...
          wv.dev.set_adsr(i,800,1300,1,4400)
        # 3. Configure Reverb
        rev.set_params(room_size=0.55, brightness=0.7, predelay_ms=215.0, transition_ms=0, mix=0.4)

        print("Starting Reverb Test...", file=vs)
        # Play a test pattern
        # A Major scale arpeggio
        p.add(wv, "<<c3 a2> d2min7 c#2min g#2sev>/2")
        p.add(wv, "<mrp_:0.8:2000 mrp_:0.2:2000>/2")
