...
import math
import random
from pie import Pie, Pattern, PieWavetable, PieSampler
...
def main(vs, args):
    ...
    with PieSampler(16) as drums, PieWavetable(1) as bass_synth:
        ...
        # Schedule the song structure natively
        p.schedule_update(4, idx_d, d_beat_minimal + ", " + perc)
        p.schedule_update(8, idx_d, d_beat_full + ", [2*16]")
        p.schedule_update(8, idx_b, b_loop)
        p.schedule_update(12, idx_d, d_beat_basic)
        p.schedule_remove(12, idx_b)
        
        with p:
          ct = 0
          last_msg_cycle = -1
          while p.playing_cycle < 16:
            p.process_event()
            
            cur_cycle = int(p.playing_cycle)
            if cur_cycle != last_msg_cycle:
              if cur_cycle == 4:
              ...
              last_msg_cycle = cur_cycle
              
            if ct & 1 == 0:
              print(f"[Cycle {cur_cycle}] Progressive minimal techno...", end="\r", file=vs)
            ct += 1
            ret = vs.v.read_nb(1)
            if ret and ret[0] > 0:
                break
