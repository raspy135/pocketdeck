...
class mouse:
  ...
  def update(self):
    ...
    if touching:
      ...
      self.point[0] = 0 if self.point[0] < 0 else self.point[0]
      self.point[1] = 0 if self.point[1] < 0 else self.point[1]
      self.lastpoint[0] = x
      self.lastpoint[1] = y
  ...
  #   Bit 1: Left button
  #   Bit 2: Right button
  def get_point(self,ret): 
    ret[0] = self.point[0] 
    ret[1] = self.point[1]
    ret[2] = self.tap | (self.lclick << 1) | (self.rclick << 2)
    self.rclick = 0
    self.lclick = 0
    if self.tap:
      self.tap = 0
